...
class DiaryAdapter(var diaryViewModel: DiaryViewModel) :
    ...
    override fun getItemCount(): Int {
        return diaryViewModel.allPlans.size
    }

    var onEndOfListReached: (() -> Unit)? = null

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val plan = diaryViewModel.allPlans.get(position)
        ...
        if (position == diaryViewModel.allPlans.size - 1) {
